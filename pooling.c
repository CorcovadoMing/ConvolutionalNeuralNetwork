			...
			for(j = 0; j < result_size; j += 1) {
				...
				for(m = 0; m < factor; m += 1) {
					for(n = 0; n < factor; n += 1) {
						if (target->feature[stack]->element[i*target->feature[stack]->size*factor+j*factor+m*target->feature[stack]->size+n] > max_element) {
	...
	for (stack = 0; stack < target->size; stack += 1) {
		...
		for(i = 0; i < result_size; i += 1) {
			for(j = 0; j < result_size; j += 1) {
				...
				for(m = 0; m < factor; m += 1) {
					for(n = 0; n < factor; n += 1) {
						sum += target->feature[stack]->element[i*target->feature[stack]->size*factor+j*factor+m*target->feature[stack]->size+n];
					}
				}
	...
	for (stack = 0; stack < target->size; stack += 1) {
		...
		for(i = 0; i < result_size; i += 1) {
			for(j = 0; j < result_size; j += 1) {
				...
				for(m = 0; m < factor; m += 1) {
					for(n = 0; n < factor; n += 1) {
						if (target->feature[stack]->element[i*target->feature[stack]->size*factor+j*factor+m*target->feature[stack]->size+n] < min_element) {
							min_element = target->feature[stack]->element[i*target->feature[stack]->size*factor+j*factor+m*target->feature[stack]->size+n];
						}
					}
				}
